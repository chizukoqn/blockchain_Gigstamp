...
import { Shield, Star, Award, MessageSquare, User, Clock, ChevronDown, ChevronUp } from 'lucide-react';
...
export function WorkerProfile({ workerAddress }: WorkerProfileProps) {
  const { feedbacks, getWorkerStats, language } = useApp();
  ...
  const stats = getWorkerStats(workerAddress);
  const workerFeedbacks = useMemo(() => {
    return feedbacks
      .filter((f) => f.workerId?.toLowerCase() === workerAddress.toLowerCase())
      .slice(0, 3);
  }, [feedbacks, workerAddress]);
  ...
  useEffect(() => {
    ...
    const fetchOnchainData = async () => {
      try {
        ...
        // Fetch reputation
        const score = await contract.reputationScore(workerAddress);
        if (mounted) setReputation(Number(score));

        // Fetch badges
        const badgeData = await contract.getBadges(workerAddress);
        ...
        const filter = contract.filters.ScoreChanged(workerAddress);
        ...
        console.error('Failed to fetch worker profile data:', err);
      } finally {
    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm overflow-hidden">
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-gray-900 font-bold text-lg">{t.profile_worker_profile}</h3>
            <p className="text-gray-500 font-mono text-xs">{workerAddress.slice(0, 6)}...{workerAddress.slice(-4)}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-bold text-gray-900">{stats.averageRating ? stats.averageRating.toFixed(1) : '—'}</span>
          </div>
          <p className="text-[10px] text-gray-500 uppercase font-bold">{t.profile_rating}</p>
        </div>
        <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
          <div className="flex items-center gap-2 mb-1">
            <Award className="w-4 h-4 text-blue-500" />
            <span className="text-sm font-bold text-gray-900">{stats.totalJobsCompleted}</span>
          </div>
      ...
      {badges.length > 0 && (
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {badges.map((b, i) => (
              <span key={i} className="px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 text-[10px] font-bold border border-emerald-100">
      ...
      {/* Reputation History - The "Reasons" */}
      <div className="mb-6">
        <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2 px-1">
      ...
      <div>
        <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2 px-1">
          <MessageSquare className="w-3 h-3" /> {t.profile_client_feedbacks}
        </h4>
        <div className="space-y-3">
          {workerFeedbacks.length > 0 ? (
            workerFeedbacks.map((f, i) => (
              <div key={i} className="p-3 bg-white border border-gray-100 rounded-xl shadow-sm">
                <div className="flex items-center gap-1 mb-1.5">
                  {[...Array(5)].map((_, idx) => (
                    <Star key={idx} className={`w-3 h-3 ${idx < f.rating ? 'text-amber-500 fill-amber-500' : 'text-gray-200'}`} />
                  ))}
                </div>
                <p className="text-[11px] text-gray-700 leading-relaxed italic">"{f.comment || '...'}"</p>
              </div>
            ))
          ) : (
            <p className="text-[10px] text-gray-400 italic px-1">{t.profile_no_feedback}</p>
          )}
        </div>
      </div>
    </div>
